...
public class NewMain extends OpMode {
    ...
    ToggleButtonReader hangButton;
    ToggleButtonReader autoOrientButton;

    ToggleButtonReader clawButton;

    ToggleButtonReader retractButton;
    ToggleButtonReader alignReader;
    ToggleButtonReader modeButton;

    ToggleButtonReader dpadUp;
    ...
    public void init() {
        ...
        hangButton = new ToggleButtonReader(
                driverOp, GamepadKeys.Button.X
        );

        autoOrientButton = new ToggleButtonReader(
        ...
        retractButton = new ToggleButtonReader(
                driverOp, GamepadKeys.Button.A
        );
        alignReader = new ToggleButtonReader(
                driverOp2, GamepadKeys.Button.A
        );
...
//                    Pika.outtakeSlides.getCurrentPosition() < OuttakeSlides.TurnValue.MAX_EXTENSION_DOWN.getTicks());
    ...
    public void stop() {
        ...
        Pika.arm.setTargetPosition(Arm.ArmPos.STUPID_TAIL.getPosition());
        while (Pika.arm.isFinished() && Pika.outtakeSlides.isFinished()) {
            Pika.arm.update();
            Pika.outtakeSlides.update();
        }
    }
